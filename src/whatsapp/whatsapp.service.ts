...
export class WhatsAppService {
  ...
  private chatJidShortLabel(jid: string): string {
    const id = jid.replace(/@(g\.us|newsletter)$/i, '');
    return id.length > 16 ? `${id.slice(0, 16)}…` : id;
  }

  private isNewsletterJid(jid: string): boolean {
    return /@newsletter$/i.test((jid || '').trim());
  }

  private pickChatTitle(o: Record<string, unknown>): string {
    const fields = [
      o.subject,
      o.name,
      o.pushName,
      o.subjectName,
      (o.thread_metadata as Record<string, unknown> | undefined)?.name,
    ];
  ...
  private parseNewsletterJidItem(
    item: unknown,
  ): { groupJid: string; title: string } | null {
    if (!item || typeof item !== 'object') return null;
    const o = item as Record<string, unknown>;
    const jidRaw =
      o.remoteJid ?? o.id ?? o.jid ?? o.groupJid ?? (o.key as Record<string, unknown>)?.remoteJid;
    if (typeof jidRaw !== 'string' || !this.isNewsletterJid(jidRaw)) return null;
    const groupJid = jidRaw.trim();
    return { groupJid, title: this.pickChatTitle(o) };
  }

  private normalizePaginatedRecords(data: unknown): unknown[] {
    if (!data || typeof data !== 'object') return [];
    const o = data as Record<string, unknown>;
    if (Array.isArray(o.records)) return o.records;
    if (Array.isArray(o.data)) return o.data;
    return this.normalizeEvolutionGroupsPayload(data);
  }

  /**
   * Canais (@newsletter): `POST /chat/findChats` (nomes) + `POST /chat/findChannels` (JIDs
   * detetados no histórico; pushName costuma vir vazio na Evolution).
   */
  async fetchInstanceChannels(
    instance?: string,
    ...
    const mergeItem = (parsed: { groupJid: string; title: string } | null) => {
      if (!parsed) return;
      const prev = byJid.get(parsed.groupJid) ?? '';
      const title = parsed.title || prev;
      byJid.set(parsed.groupJid, title);
    };

    try {
      const chatsRes = await fetch(
        `${base}/chat/findChats/${encodeURIComponent(inst)}`,
        {
          method: 'POST',
          headers,
          body: JSON.stringify({}),
      ...
      if (chatsRes.ok) {
        const data: unknown = await chatsRes.json().catch(() => null);
        const raw = this.normalizeEvolutionGroupsPayload(data);
        for (const item of raw) {
          mergeItem(this.parseNewsletterJidItem(item));
        }
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      this.logger.warn(`findChats (canais) falhou (${inst}): ${msg}`);
    }

    try {
      const channelsRes = await fetch(
        `${base}/chat/findChannels/${encodeURIComponent(inst)}`,
        {
          method: 'POST',
          headers,
      ...
      if (channelsRes.ok) {
        const data: unknown = await channelsRes.json().catch(() => null);
        for (const item of this.normalizePaginatedRecords(data)) {
          mergeItem(this.parseNewsletterJidItem(item));
        }
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      this.logger.warn(`findChannels falhou (${inst}): ${msg}`);
    }

    const parsed = [...byJid.entries()].map(([groupJid, title]) => ({
      groupJid,
      title: title || this.chatJidShortLabel(groupJid),
    }));
  ...
  /** Grupos + canais da instância principal (para o dropdown admin). */
  async fetchInstanceGroupTargets(
    instance?: string,
  ): Promise<
    { groupJid: string; title: string; kind: 'group' | 'channel' }[]
  > {
    const inst = (instance || this.getPrimaryInstanceName()).trim();
    const [groups, channels] = await Promise.all([
      this.fetchInstanceGroups(inst),
      this.fetchInstanceChannels(inst),
    ]);
    const seen = new Set<string>();
    const out: { groupJid: string; title: string; kind: 'group' | 'channel' }[] =
      [];
    for (const g of groups) {
      if (seen.has(g.groupJid)) continue;
      seen.add(g.groupJid);
      out.push({ ...g, kind: 'group' });
    }
    for (const c of channels) {
      if (seen.has(c.groupJid)) continue;
      seen.add(c.groupJid);
      out.push({ ...c, kind: 'channel' });
    }
    return out.sort((a, b) =>
      a.title.localeCompare(b.title, 'pt-PT', { sensitivity: 'base' }),
    );
      ...
      for (let i = 0; i < missing.length; i += enrichBatch) {
        ...
        await Promise.all(
          chunk.map(async (g) => {
            ...
            g.title = subject || this.chatJidShortLabel(g.groupJid);
          }),
        );
      }
      for (const g of parsed) {
        if (!g.title) g.title = this.chatJidShortLabel(g.groupJid);
      }
